...
- `layouts/index.html` — homepage "Latest Articles" grid, via `resolve-image-src.html` directly (no body-image fallback), so an article with no `featured_image` shows no thumbnail there. This is deliberate — don't switch it to `resolve-thumb.html`.
