...
- **Contact form** — markup lives in `content/contact.md`; the submit handler (calling the `send-contact-email` Edge Function) is in `static/js/main.js`.
...
To change the backend (e.g. swap Supabase projects or endpoints), update the `fetch(...)` URL and `Authorization` header in `static/js/main.js` (contact form) and `layouts/blog/single.html` (newsletter).
