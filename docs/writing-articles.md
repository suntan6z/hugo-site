...
Place the partner's logo file directly in the article's bundle folder alongside `index.md` (e.g. `content/blog/my-slug/partnerlogo.png`) and reference it by filename in `partner_logo_url`. The generic EU-funding badge itself is shared across all Erasmus+ articles and lives at `assets/eufundedlogo.png` — you don't need to add that one yourself.
